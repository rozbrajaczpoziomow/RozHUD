...
class Help : CommandBase() {
	...
	override fun execute(server: MinecraftServer, sender: ICommandSender, args: Array<out String?>) {
		...
		sender.reply("/rozutils stats - show TPS/MSPT stats for the server, your current dimension and the dimension with the longest MSPT")
		sender.reply("/rozutils stats [...dimensionIds] - show TPS/MSPT stats for the specified dimensions")
		sender.reply("/rozutils entities - show entity stats for the server, your current dimension and the dimension with the most entities")
		...
		sender.reply("/rozutils players - show currently online players, operator status, gamemode, dimension and coordinates")
...
private inline fun ICommandSender.reply(msg: String) = sendMessage(TextComponentString(msg))
private inline fun ICommandSender.reply(msg: String, command: String) = replyOther(msg, "/rozutils $command")
private inline fun ICommandSender.replyOther(msg: String, command: String) = sendMessage(TextComponentString(msg).setStyle(Style().setClickEvent(ClickEvent(ClickEvent.Action.RUN_COMMAND, command))))
